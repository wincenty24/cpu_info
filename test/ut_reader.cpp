...
void prepare_ref(pre_cpu_info_t* pci){
...
void compare_char(char* a, char* b){
    int32_t are_eq = strcmp(a, b);
    ASSERT_FALSE(are_eq);
}

void compare_pre_cpu_info_t(pre_cpu_info_t* pci, pre_cpu_info_t* pci_ref, int32_t n_pre_cpu_info_t){
    for(int32_t i =0; i < n_pre_cpu_info_t; i++){
        compare_char(pci_ref->pre_cpu_str, pci->pre_cpu_str);
        pci++;
        pci_ref++;
    }
}
...
TEST(READER,read_stat ){
    ...
    prepare_ref(pci_ref);
    compare_pre_cpu_info_t(pci, pci_ref, nproc);
